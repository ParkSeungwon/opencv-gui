...
{
	float ratio1 = this->cols / this->rows;
	float ratio2 = w / h;
	if(ratio1 > ratio2) {
		int dest_width = this->rows * ratio2;
		int cut = (this->cols - dest_width) / 2;
		cv::resize((*this)(Rect(cut, 0, dest_width, this->rows)), *this, cv::Size(w, h), cv::INTER_LINEAR);
	} else if(ratio1 < ratio2) {
		int dest_height = this->cols / ratio2;
		int cut = (this->rows - dest_height) / 2;
		cv::resize((*this)(Rect(0, cut, this->cols, dest_height)), *this, cv::Size(w, h), cv::INTER_LINEAR);
...
string CVMat::info()
{
	...
	stringstream ss;
	ss << cols << "x" << rows << ", " << c[depth()] << 'C' << channels();
	cout << ss.str() << endl;
	return ss.str();
}
...
void CVMat::gray()
{
	if(empty()) return;
	switch(channels()) {
		case 3: cvtColor(*this, *this, COLOR_BGR2GRAY); break;
		case 4: cvtColor(*this, *this, COLOR_BGRA2GRAY); break;
	}
}
